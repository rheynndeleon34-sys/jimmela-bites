...
const AdminDashboard = () => {
  ...
  // Filter out incomplete orders and validate data
  const validOrders = useMemo(() => {
    if (!Array.isArray(ordersData)) return [];
    
    // Filter orders that have required fields
    return ordersData.filter(order => {
      ...
      const hasRequiredFields = 
        ...
        order.id &&
        order.customer &&
        Array.isArray(order.items) && // This is key - ensures items is an array
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        {ordersLoading ? (
          ...
          <div className="px-6 py-12 text-center">
            ...
            <p className="text-muted-foreground">No valid orders found</p>
            {ordersData && ordersData.length > 0 && (
              <p className="text-xs text-muted-foreground mt-2">
                Found {ordersData.length} order(s) but they're missing required fields
              </p>
            )}
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 text-muted-foreground hidden md:table-cell max-w-[250px]">
                      <span className="truncate block text-xs">
                        {o.items && Array.isArray(o.items) && o.items.length > 0
                          ? o.items.map((item) => item?.productName || 'Unknown').join(", ")
                          : "No items"}
                      </span>
                    </td>
                    <td className="px-6 py-4">
